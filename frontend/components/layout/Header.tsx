...
// Componente que verifica token nos cookies antes de redirecionar
function LoginButton() {
  const router = useRouter()

  const handleLoginClick = async () => {
    // Verificar se há token válido nos cookies
    ...
    if (response.ok) {
      // Token válido, ir direto para dashboard
      router.push('/dashboard')
    } else {
      // Sem token ou token inválido, ir para página de login
      router.push('/login')
    }
  }
